...
class Join(Operator):
  ...
  def cost(self, estimated):
    lhsNum = self.lhsPlan.cardinality(estimated)
    rhsNum = self.rhsPlan.cardinality(estimated)
    pageSize = self.storage.bufferPool.pageSize
    lhsPages = lhsNum / pageSize
    rhsPages = rhsNum / pageSize

    if self.joinMethod == "nested-loops":
      return lhsPages + lhsNum * rhsPages

    elif self.joinMethod == "block-nested-loops":
      memoryCapacity = self.storage.bufferPool.poolSize / pageSize
      return lhsPages + (lhsPages / (memoryCapacity - 2)) * rhsPages

    elif self.joinMethod == "indexed":
      return lhsPages + lhsNum * (self.selectivity(estimated) / pageSize)

    elif self.joinMethod == "hash":
      return 3 * (lhsPages + rhsPages)
