...
class GroupBy(Operator):
  ...
  def cost(self, estimated):
    ...
    aggCost = partitionCost * self.selectivity(estimated)
    return partitionCost + aggCost

  def selectivity(self, estimated):
    attrDisValues = []

    for (attr, type) in self.groupSchema.schema():
      if estimated:
        if attr in self.disValuesEstimated.keys():
          attrDisValues.append(self.numDisValues(attr, estimated=True))
      else:
        if attr in self.disValuesEstimated.keys():
          attrDisValues.append(self.numDisValues(attr, estimated=False))

    if len(attrDisValues) == 0:
      return 1

    return max(attrDisValues) / self.cardinality(estimated)