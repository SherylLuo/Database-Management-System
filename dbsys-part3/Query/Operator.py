class Operator:
  ...
  def initializeStatistics(self):
    ...
    self.actualMaxima = {}
    self.disValuesEstimated = {}
    self.disValuesActual = {}
  ...
  def updateStats(self, outputTuple, estimated):
    for (field, fieldType) in self.schema().schema():
      ...
      if estimated:
        if not field in self.estimatedMaxima:
          self.estimatedMaxima[field] = [fieldValue, fieldValue]
          self.disValuesEstimated[field] = (fieldValue,)
        else:
          self.disValuesEstimated[field].add(fieldValue)
        ...
        if not field in self.actualMaxima:
          self.actualMaxima[field] = [fieldValue, fieldValue]
          self.disValuesActual[field] = (fieldValue,)

        else:
          self.disValuesActual[field].add(fieldValue)
          maxima = self.actualMaxima[field]
          newMaxima = [max(fieldValue, maxima[0]), min(fieldValue, maxima[1])]
          self.actualMaxima[field] = newMaxima


  def numDisValues(self, field, estimated):
    if estimated:
      return len(self.disValuesEstimated[field])
    else:
      return len(self.disValuesActual[field])
